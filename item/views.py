...
from grn.models import Grn, Stock, StockOut
from store.models import Store

from .forms import ItemForm, ItemTypeForm, UomForm
        ...
        try:
            ...
            if item_type_serializer.is_valid():
                if ItemType.objects.filter(
                    name__iexact=item_type_serializer.data["name"]
                ):
        ...
        try:
            ...
            if item_serializer.is_valid():
                if Item.objects.filter(
                    name__iexact=item_serializer.data["name"]
                ).exclude(id=payload.get("id")):
...
class UomDetailsView(View):
    ...
    def post(self, request):
        try:
            ...
            if uom_serializer.is_valid():
                if Uom.objects.filter(name__iexact=uom_serializer.data["name"]).exclude(
                    id=payload.get("id")
                ):
                    message = "Uom already exists"
                    messages.warning(request, message)
